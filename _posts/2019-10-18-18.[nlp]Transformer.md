...
- Encoder를 하나의 '층'으로 생각해보자. 하나의 Encoder는 총 2개의 sub layer로 구성된다 
  - 1 ) Self Attention 
...
d_model에서, d_model / num_heads으로 차원축소가 이루어진다! 

( num_heads개 병렬 Attention을 동시에 수행! )

병렬 처리를 하는 이유는, 보다 다양한 특징을 capture하기 위해서이다. 
...
여기에서 사용되는 parameter들 (W1, b1, W2, b2)는 문장 별로 모두 동일하지만, Encoder층 마다는 다른 값을 가진다. 아래 그림을 보면, 지금까지의 연산이 어떻게 이루어지는지 쉽게 이해할 수 있다.
