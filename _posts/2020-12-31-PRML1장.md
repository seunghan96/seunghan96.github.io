...
$$\begin{aligned}
p_{y}(y) &=p_{x}(x)\left\mid\frac{\mathrm{d} x}{\mathrm{~d} y}\right\mid \\
&=p_{x}(g(y))\left\midg^{\prime}(y)\right\mid
\end{aligned}$$
...
- $$\mathbb{E}\left[L_{q}\right]=\iint\midy(\mathbf{x})-t\mid^{q} p(\mathbf{x}, t) \mathrm{d} \mathbf{x} \mathrm{d}$$
