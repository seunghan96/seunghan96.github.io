...
excerpt: stochastic optimization, control variate, MFVI, Stochastic search Variational Bayes
---
...
위 ELBO가 intractable할 경우? $$g\left(\theta_{i}, \xi\right)$$를 도입하여 $$f_{j}$$를 대체한다!

( 여기서 $$f_{j}\left(\theta_{i}\right) \geq g\left(\theta_{i}, \xi\right)$$ )
...
이번에는 ELBO를 direct하게 optimize하기 위한 stochastic search방법에 대해 이야기할 것이다.

( 편의를 위해 indices 생략 )

- $$f$$ : $$\theta$$에 대한 intractable 함수
...
위의 2개의 term중, intractable한 1번째 term은 **log-derivative trick**를 사용하여 아래와 같이 풀 수 있다.
...
이 값이 클 경우, 더 많은 sample수가 필요 하고, 이는 결국 알고리즘의 속도 저하로 이어진다. 따라서 우리는 "variance reduction"을 해야하는데, 이를 위해 **control variate**를 도입한다.
...
​	 $$\theta^{(s)} \stackrel{\text { iid }}{\sim} q(\theta \mid \psi)$$ for $$s=1, \ldots, S$$.

<br>

## 4.2 The stochastic searh case

위에서 우리가 한 것은 $$g(\theta)$$를 사용하여 $$f(\theta)$$를 대체한 것이다.
