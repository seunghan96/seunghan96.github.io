...
이를 해결하기 위한 큰 두 가지 접근 법에는 **Frequentist Framework**와 **Bayesian Framework**가 있다.
...
이렇게 해서 나오게 된 결과들을 종합(ensemble)한다. ( 하나의 추정된 $$\theta_{ML}$$ 를 사용하는 것 보다, 여러 알고리즘을 사용하여 추정된 여러 개의 $$\theta$$를 ensemble하는 것이 주로 더 좋은 결과를 보인다 )
...
위 식을 개념적으로 나타내면 $$Posterior = \frac{Likelihood \times Prior}{Evidence}$$과 같고, 즉 여기서 Evidence를 구하기 어려울 수 있다는 것이다. 하지만, 특정한 경우에 우리는 이 문제를 쉽게 풀 수 있다. 우리가 특정 Likelihood에 대해서, Prior를 특정 분포로 사용했을 때, Posterior도 이와 동일한 분포를 띄는 경우이다. 이런 경우를 우리는 **Conjugate**하다고 표현한다.
...
결론 부터 얘기하면, likelihood가 **Bernoulli Distribution**일 때, prior가 **Beta Distribution**일 경우 이 둘은 서로 conjugate한다. 다음 식을 통해 확인해보자
...
( 여기서 $$B(a,b) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) + \Gamma(\beta)} 이다)$$
