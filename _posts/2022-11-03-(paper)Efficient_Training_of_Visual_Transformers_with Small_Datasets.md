...
- alternative to standard CNN
- inspired by Transformer [55]
- pioneering work : ViT 
...
- main difference with respect to CNNs

  ( CNN : receptive field of the kernels locally limits the type of relations )
...
- common idea : HYBRID ( = mix **convolutional layers with attention layers** )
  ...
  - enjoy the advantages of both paradigms : 
    - attention layers ) model long-range dependencies
    - convolutional operations ) emphasize the local properties
...
-  **auxiliary self-supervised pretext task** and a corresponding **loss function** to regularize training in a small training set or few epochs regime
...
2. propsoe a **relative localization auxiliary task** for VT training regularization
