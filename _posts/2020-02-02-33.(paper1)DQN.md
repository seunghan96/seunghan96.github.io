...
- $$\theta \leftarrow \theta+\eta \frac{\partial \mathcal{L}\left(s, a, r, s^{\prime}\right)}{\partial \theta} $$.
  - $$\mathcal{L}\left(s, a, r, s^{\prime}\right)=\left\|_{r+\gamma \max } Q_{\theta}\left(s^{\prime}, a^{\prime}\right)-Q_{\theta}(s, a)\right\|_{2}$$.
...
Target Network $$\hat{Q}\left(s, a ; \theta^{-}\right)$$의 parameter $$\theta^{-}$$는 $$C$$ step 마다, 

Q Network $$Q(s, a ; \theta)$$의 parameter $$\theta$$로 동일하게 update된다. ( paper : set $$C=10, 000$$ )
