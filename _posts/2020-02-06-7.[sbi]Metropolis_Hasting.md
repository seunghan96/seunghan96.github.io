...
[Step 1] Let our current value $$z^t$$. Take a sample $$z^{*} \sim q(z^{*}|z^t)$$ .
...
It is a ratio, $$r(z^{*}\mid z^t) $$ = $$\frac {q(z^t \mid z^{*}) \cdot P(z^{*})} {q(z^{*} \mid z^{t}) \cdot P(z^{t})}$$, and we want this ratio to become 1. As you can see, making this '1' means that we want our Markov Chain to be 'reversible', which makes $$\pi$$ a stationary distribution. So if the numerator is bigger than the denominator, we reduce the numerator, and vise versa. If we iterate this many times, we will get an (approximately) proper proposal distribution.
