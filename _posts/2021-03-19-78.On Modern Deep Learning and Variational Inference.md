...
$$\mathcal{L}_{\text {dropout }}:=\frac{1}{N} \sum_{i=1}^{N} E\left(\mathbf{y}_{i}, \widehat{\mathbf{y}}_{i}\right)+\lambda \sum_{i=1}^{L}\left(\left\mid\mathbf{W}_{i}\right\mid_{2}^{2}+\left\mid\mathbf{b}_{i}\right\mid_{2}^{2}\right)$$.
